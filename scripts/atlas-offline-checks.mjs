...
const server = spawn('python3', ['-u', '-c', [
  ...
  "   body = (ROOT / 'app/offline-worker.mjs').read_text().replace('v6', 'v7').encode()",
...
try {
  ...
  const cachePaths = await page.evaluate(async () => {
    const cache = await caches.open('kowlo-atlas-shell-v6');
  ...
  assert(await page.evaluate(async () => (await caches.keys()).includes('kowlo-atlas-shell-v6')));
  ...
  await page.goto(`${origin}/app/index.html#atlas`); await ready();
  await page.waitForFunction(async () => !(await caches.keys()).includes('kowlo-atlas-shell-v6'));
  assert.deepEqual((await page.evaluate(() => caches.keys())).sort(), ['hk-photo-diagnostic-shell-sentinel', 'kowlo-atlas-shell-v7']);
  ...
  const retained = await page.evaluate(async () => (await (await caches.open('kowlo-atlas-shell-v7')).keys()).map(request => new URL(request.url).pathname).sort());
