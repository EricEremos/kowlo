...
- GitHub: `EricEremos/hong-kong-footprints` was created and verified private and empty. Local origin is connected; code has not been pushed.
- Figma: the new Hong Kong Footprints file is connected and contains editable designs. Four revised automatic-atlas frames were exported and visually inspected; user design review remains pending. See [the design evidence](evidence/automatic-atlas-review.md).
- Supabase: EricEremos's Org in Singapore is approved. The CLI connection works; the new cloud project has not been created. The first private-journal migration passed 58 isolated local PostgreSQL checks, which do not establish hosted JWT/API behavior. See [the database evidence](evidence/private-journal-storage.md).
- Local extraction and district experiments have automated and desktop-browser evidence. Physical-device library access, actual photo-colour extraction, hosted sync and the complete production interface remain unverified or unimplemented.
